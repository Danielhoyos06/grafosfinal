...
std::vector<long> Grafo<TVertex,TCost>::Dijkstra(long start, long end) const
{
  ...
  while (!pq.empty())
  {
    ...
    for (std::pair<long, TCost> edge : adj[u])
{
    long v = edge.first;
    TCost w = edge.second;
...
// La estructura general de este método de búsqueda de ruta de costo mínimo
// fue generada con apoyo de una herramienta de IA (ChatGPT) a partir del prompt:
// "Ayúdame a implementar el algoritmo de Dijkstra en C++ para un grafo no dirigido
// usando plantillas y sin utilizar la palabra reservada auto".
